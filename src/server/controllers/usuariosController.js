...
// Adiciona amigo para o usuário logado
...
exports.removerAmigo = async (req, res) => {
    try {
        ...
        // Remover amizade em ambas as direções (se existir)
        const [result] = await db.query(
            'DELETE FROM Amizade WHERE (fk_Usuario_id_usuario = ? AND fk_Usuario_id_usuario_ = ?) OR (fk_Usuario_id_usuario = ? AND fk_Usuario_id_usuario_ = ?) LIMIT 2',
        ...
        return res.status(200).json({ message: 'Amizade removida para ambos os usuários.' });
    } catch (err) {
